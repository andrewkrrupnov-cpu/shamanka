...
# Приветствий в раскладах быть НЕ должно (требование владельца).
GREETINGS = ["здравствуй", "здравствуйте", "привет", "приветствую", "добрый день",
             "доброе утро", "добрый вечер", "дорогая", "дорогой"]
...
def check_reading(text: str, spread_id: str, spread: dict, cards) -> dict:
    ...
    head = low.lstrip()[:60]  # ищем приветствие в самом начале трактовки
    greeted = any(g in head for g in GREETINGS)
    five_blocks = all(re.search(rf"(?m)^\s*\**\s*{n}[.\)]", text) for n in range(1, 6)) \
        or ("Наставление" in text and "Расклад" in text)
    chars = len(text)
    fits = chars <= TELEGRAM_LIMIT
    return {
        ...
        "ок": (not forbidden_hits) and bool(five_blocks)
              and (not greeted) and fits,
    }
...
# --------------------------------------------------------------------------- #
JUDGE_SYSTEM = (
    "Ты — редактор Таро-бота. Голос бота — Саира, уверенная мать-провидица из мира "
    "песков: говорит властно и чуть наставительно, от себя, без приветствий. Оцени "
    "трактовки (1–5 каждой): выдержан ли этот характер; уверенность БЕЗ буквальных "
    "предсказаний смерти, тяжёлой болезни и катастроф (тяжёлые карты — как перемены); "
    "отсутствие приветствий; собранность и польза. Дай краткую таблицу и 2–3 фразы "
    "вывода про ровность голоса. По-русски, тире только «–»."
)
...
def main():
    ...
    for i, ex in enumerate(EXAMPLES, 1):
        ...
        print(f"  [{flag}] запрещённые={chk['запрещённые']} приветствие={chk['приветствие']} "
              f"тяжёлые={chk['тяжёлые_карты']} 5_блоков={chk['5_блоков']} "
              f"{chk['символов']}симв/лимит={chk['в_лимит']}")
    ...
    for i, (ex, spread, cards, text, layout) in enumerate(interps, 1):
        ...
        lines += [
            ...
            "```",
            "",
            f"_Проверка: {'OK' if chk['ок'] else 'ВНИМАНИЕ'} · запрещённые "
            f"{chk['запрещённые'] or 'нет'} · приветствие "
            f"{'есть!' if chk['приветствие'] else 'нет'} · тяжёлые карты "
            f"{chk['тяжёлые_карты'] or 'нет'} · 5 блоков "
            f"{'да' if chk['5_блоков'] else 'нет'} · {chk['символов']} символов "
            f"({'влезает' if chk['в_лимит'] else 'ПРЕВЫШЕНИЕ'})_",
            "",
            text, "", "---", "",
        ]
    if judge_text:
